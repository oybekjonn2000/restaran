...
@Component({
  ...
  template: `
    <div class="categories-admin animate-in">
      <div class="page-header">
        <div>
          <h1 class="page-title">📁 Taom Kategoriyalari</h1>
          <p style="color: var(--text-muted); font-size: 0.875rem;">Restoranlar bo'yicha toifalarni yaratish va boshqarish</p>
        </div>
      ...
      <div class="filters-bar">
        <div class="search-box">
          <span>🔍</span>
          <input [(ngModel)]="searchQ" (input)="onSearch()" type="text" placeholder="Kategoriya qidiring..." class="search-input" id="admin-cat-search">
        </div>

        <div class="filter-group">
          <label style="font-size: 0.85rem; color: var(--text-muted);">🏪 Restoran:</label>
          <select [(ngModel)]="selectedRestaurantId" (change)="onFilterChange()" class="form-control filter-select">
          ...
          @for (c of categories(); track c.id) {
            <div class="category-admin-card" [id]="'cat-card-' + c.id">
              ...
              <div class="cat-details">
                ...
                @if (c.restaurant) {
                  <p class="cat-meta">🏪 Restoran: <strong>{{ c.restaurant.name }}</strong></p>
                } @else {
                  <p class="cat-meta text-danger">⚠️ Restoran biriktirilmagan</p>
                }
            ...
            <div class="modal-body">
              <form [formGroup]="catForm" (ngSubmit)="saveCategory()">
                ...
                <div class="form-group">
                  ...
                  <select formControlName="restaurantId" class="form-control" id="cat-form-restaurant">
                    <option value="">Restoran tanlang...</option>
                    @for (r of restaurants(); track r.id) {
                      <option [value]="r.id">{{ r.name }}</option>
                    }
                  </select>
                  @if (catForm.get('restaurantId')?.invalid && catForm.get('restaurantId')?.touched) {
                    <span class="field-error">Restoran tanlanishi shart</span>
                  }
                </div>
  ...
  `,
  styles: [`
    .categories-admin { max-width: 1000px; }
    .filters-bar {
      display: flex;
      flex-wrap: wrap;
      gap: 16px;
...
export class AdminCategoriesComponent implements OnInit {
  ...
  loadRestaurants(): void {
    this.orderService.getRestaurants().subscribe({
      next: (rests) => this.restaurants.set(rests),
      error: () => this.showToast('❌ Restoranlarni yuklab bo\'lmadi', true)
    });
  }

  load(): void {
    this.loading.set(true);
    this.foodService.getCategoriesAdminPaginated({
  ...
  openEditForm(cat: Category): void {
    ...
    this.catForm.patchValue({
      ...
      restaurantId: cat.restaurant?.id || ''
    });
    this.showForm.set(true);
  }
  ...
  saveCategory(): void {
    ...
    const payload = {
      ...
      restaurantId: +this.catForm.value.restaurantId
    };

    const req$ = this.editId()
      ? this.foodService.updateCategory(this.editId()!, payload)
      : this.foodService.createCategory(payload);
  ...
  deleteCategory(id: number): void {
    ...
    this.foodService.deleteCategory(id).subscribe({
      next: () => {
        this.load();
