...
@Component({
  ...
  template: `
    <div class="menu-admin animate-in">
      <div class="page-header">
        <div>
          <h1 class="page-title">🍕 Menyu Boshqaruvi</h1>
          <p style="color: var(--text-muted); font-size: 0.875rem;">Taomlar qo'shish, o'zgartirish, o'chirish</p>
        </div>
  ...
  styles: [`
    ...
    .modal-card {
      ...
      border-radius: var(--radius-lg); width: 100%; max-width: 520px; max-height: 90vh; overflow-y: auto;
    }
    .modal-header {
...
export class AdminMenuComponent implements OnInit {
  ...
  saveFood(): void {
    ...
    const req$ = this.editId()
      ? this.foodService.updateFood(this.editId()!, data)
  ...
  toggleAvail(food: Food): void {
    this.foodService.toggleFoodAvailability(food.id).subscribe({
      next: (updated) => {
